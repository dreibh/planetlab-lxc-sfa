...
class ManifoldUploader:
    ...
    def password(self):
        if not self._password:
            ...
            self._password = getpass.getpass(
                "Enter password for manifold user %s: " % username)
    ...
    def upload(self, delegated_credential, message=None):
        ...
        auth = {'AuthMethod': 'password',
                'Username': username, 'AuthString': password}
        if not message:
            message = ""
        ...
        try:
            ...
            try:
                self.logger.debug(
                    "Using new v2 method forward+annotation@%s %s" % (platform, message))
                retcod2 = manifold.forward(query, annotation)
            except Exception as e:
                # xxx we need a constant constant for UNKNOWN, how about using
                # 1
                MANIFOLD_UNKNOWN = 1
            ...
            self.logger.error("Could not upload %s" %
                              (message if message else "credential"))
            ...
            if message:
                self.logger.error("Could not upload %s %s" % (message, e))
            else:
                self.logger.error("Could not upload credential %s" % e)
