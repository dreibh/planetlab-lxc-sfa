...
for i in range(1, 11):
    node = {'hostname': 'node' +
            str(i) + '.dummy-testbed.org', 'type': 'dummy-node', 'node_id': i}
...
users_list = []
for i in range(1, 5):
    user = {'user_name': 'user' + str(i), 'user_id': i, 'email': 'user' + str(
        i) + '@dummy-testbed.org', 'keys': ['user_ssh_pub_key_' + str(i)]}
    users_list.append(user)

DB = {'nodes_list': nodes_list, 'node_index': 11, 'slices_list': slices_list,
      'slice_index': 3, 'users_list': users_list, 'user_index': 5}
