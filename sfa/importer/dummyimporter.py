...
class DummyImporter:
    ...
    def remember_record_by_hrn(self, record):
        ...
        if tuple in self.records_by_type_hrn:
            self.logger.warning(
                "DummyImporter.remember_record_by_hrn: duplicate (%s,%s)" % tuple)
            return
    ...
    def remember_record_by_pointer(self, record):
        if record.pointer == -1:
            self.logger.warning(
                "DummyImporter.remember_record_by_pointer: pointer is void")
            return
        tuple = (record.type, record.pointer)
        if tuple in self.records_by_type_pointer:
            self.logger.warning(
                "DummyImporter.remember_record_by_pointer: duplicate (%s,%s)" % tuple)
            return
...
#        sites_by_login_base = dict ( [ ( site['login_base'], site ) for site in sites ] )
        ...
        for site in sites:
            ...
            if not site_record:
                try:
                    ...
                    self.logger.info(
                        "DummyImporter: imported authority (site) : %s" % site_record)
                    ...
                    # them.
                    self.logger.log_exc(
                        "DummyImporter: failed to import site. Skipping child records")
                    continue
            else:
            ...
            for node in nodes:
                ...
                if not node_record:
                    try:
                        ...
                        global_dbsession.commit()
                        self.logger.info(
                            "DummyImporter: imported node: %s" % node_record)
                        self.remember_record(node_record)
                    except:
                        self.logger.log_exc(
                            "DummyImporter: failed to import node")
                else:
                    # xxx update the record ...
                    pass
            ...
            for user in users:
                ...
                # return a tuple pubkey (a dummy TB key object) and pkey (a
                # Keypair object)
                ...
                try:
                    if not user_record:
                        ...
                        global_dbsession.commit()
                        self.logger.info(
                            "DummyImporter: imported person: %s" % user_record)
                        ...
                        if new_keys:
                            ...
                            if not pubkey:
                                ...
                                user_record.reg_keys = [RegKey(pubkey)]
                            self.logger.info(
                                "DummyImporter: updated person: %s" % user_record)
                    user_record.email = user['email']
                    global_dbsession.commit()
                    user_record.stale = False
                except:
                    self.logger.log_exc("DummyImporter: failed to import user %d %s" % (
            ...
            for slice in slices:
                ...
                if not slice_record:
                    try:
                        ...
                        global_dbsession.commit()
                        self.logger.info(
                            "DummyImporter: imported slice: %s" % slice_record)
                        self.remember_record(slice_record)
                    except:
                        self.logger.log_exc(
                            "DummyImporter: failed to import slice")
                else:
                    # xxx update the record ...
                ...
                slice_record.reg_researchers = \
                    [self.locate_by_type_pointer(
                        'user', user_id) for user_id in slice['user_ids']]
                global_dbsession.commit()
        ...
        for record in all_records:
            ...
            if stale:
                self.logger.info(
                    "DummyImporter: deleting stale record: %s" % record)
