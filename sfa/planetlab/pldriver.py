...
class PlDriver (Driver):
    ...
    def register(self, sfa_record, hrn, pub_key):
        ...
        if type == 'authority':
            sites = self.shell.GetSites(
                {'peer_id': None, 'login_base': pl_record['login_base']})
            if not sites:
                # xxx when a site gets registered through SFA we need to set
                # its max_slices
            ...
            slices = self.shell.GetSlices(
                {'peer_id': None, 'name': pl_record['name']})
            if not slices:
            ...
            persons = self.shell.GetPersons(
                {'peer_id': None, 'email': sfa_record['email']})
            if not persons:
                for key in ['first_name', 'last_name']:
                    if key not in sfa_record:
                        sfa_record[key] = '*from*sfa*'
                # AddPerson does not allow everything to be set
                can_add = ['first_name', 'last_name', 'title',
                           'email', 'password', 'phone', 'url', 'bio']
                add_person_dict = {k: sfa_record[k]
                                   for k in sfa_record if k in can_add}
            ...
            if 'roles' in sfa_record:
                ...
                roles = [role for role in sfa_record[
                    'roles'] if role in ['user', 'tech']]
            ...
            # Add the user's key
            if pub_key:
                self.shell.AddPersonKey(
                    pointer, {'key_type': 'ssh', 'key': pub_key})

        elif type == 'node':
            login_base = PlXrn(
                xrn=sfa_record['authority'], type='authority').pl_login_base()
            nodes = self.shell.GetNodes(
                {'peer_id': None, 'hostname': pl_record['hostname']})
            if not nodes:
    ...
    def update(self, old_sfa_record, new_sfa_record, hrn, new_key):
        ...
        if (type == "authority"):
            logger.debug(
                "pldriver.update: calling UpdateSite with {}".format(new_sfa_record))
            ...
            # this is because 'email' is a native field in the RegUser
            # object...
            ...
            if new_key:
                ...
                persons = self.shell.GetPersons(
                    {'peer_id': None, 'person_id': pointer}, ['key_ids'])
                person = persons[0]
                keys = person['key_ids']
                ...
                for key in keys:
                    if new_key == key['key']:
                        ...
                        break
                if not key_exists:
                    new_key_pointer = self.shell.AddPersonKey(
                        pointer, {'key_type': 'ssh', 'key': new_key})
    ...
    def remove(self, sfa_record):
        ...
        if type == 'user':
            persons = self.shell.GetPersons(
                {'peer_id': None, 'person_id': pointer})
    ...
    def sfa_fields_to_pl_fields(self, type, hrn, sfa_record):
        ...
        if type == "slice":
            ...
            pl_record["login_base"] = PlXrn(
                xrn=hrn, type='authority').pl_login_base()
        ...
        for record in records:
            ...
            if record['type'] == 'user':
                if 'key_ids' not in record:
                    logger.info(
                        "user record has no 'key_ids' - need to import from myplc ?")
                else:
                    pubkeys = [keys[key_id]['key']
                               for key_id in record['key_ids'] if key_id in keys]
                    record['keys'] = pubkeys
    ...
    def fill_record_hrns(self, records):
        ...
        if person_ids:
            person_list = self.shell.GetPersons(
                {'peer_id': None, 'person_id': person_ids}, ['person_id', 'email'])
            persons = list_to_dict(person_list, 'person_id')
        if slice_ids:
            slice_list = self.shell.GetSlices(
                {'peer_id': None, 'slice_id': slice_ids}, ['slice_id', 'name'])
            slices = list_to_dict(slice_list, 'slice_id')
        if node_ids:
            node_list = self.shell.GetNodes(
                {'peer_id': None, 'node_id': node_ids}, ['node_id', 'hostname'])
            nodes = list_to_dict(node_list, 'node_id')
        ...
        for record in records:
            ...
            if 'person_ids' in record:
                ...
                person_hrns = [".".join([auth_hrn, login_base, username])
            ...
            if 'site_ids' in record:
                login_bases = [sites[site_id]['login_base'] for site_id in record['site_ids']
                               if site_id in sites]
                site_hrns = [".".join([auth_hrn, lbase])
                             for lbase in login_bases]
    ...
    def fill_record_sfa_info(self, records):
        ...
        site_pis = {}
        if site_ids:
            pi_filter = {'peer_id': None, '|roles': [
                'pi'], '|site_ids': site_ids}
            pi_list = self.shell.GetPersons(
                pi_filter, ['person_id', 'site_ids'])
            for pi in pi_list:
        ...
        person_list, persons = [], {}
        person_list = self.api.dbsession().query(
            RegRecord).filter(RegRecord.pointer.in_(person_ids))
        ...
        pl_person_list = self.shell.GetPersons(
        ...
        for record in records:
            ...
            type = record['type']
            logger.info(
                "fill_record_sfa_info - incoming record typed {}".format(type))
            if (type == "slice"):
                ...
                if record['pointer'] != -1:
                    ...
                    for pointer in record.get('person_ids', []):
                        if pointer not in persons or pointer not in pl_persons:
                            # this means there is not sfa or pl record for this
                            # user
                            continue
    ...
    def update_relation(self, subject_type, target_type, relation_name, subject_id, target_ids):
        ...
        if subject_type == 'slice' and target_type == 'user' and relation_name == 'researcher':
            ...
            # it's tricky to remove any pi role here, although it might be
            # desirable
            persons = self.shell.GetPersons(
                {'peer_id': None, 'person_id': target_ids})
            for person in persons:
    ...
    def allocate(self, urn, rspec_string, expiration, options=None):
        ...
        site = slices.verify_site(
            xrn.hrn, slice_record, sfa_peer, options=options)
        # ensure slice record exists
        slice = slices.verify_slice(
            xrn.hrn, slice_record, sfa_peer, expiration=expiration, options=options)
        # ensure person records exists
        persons = slices.verify_persons(
            xrn.hrn, slice, users, sfa_peer, options=options)
        ...
        slices.verify_slice_links(
            slice, rspec.version.get_link_requests(), nodes)
    ...
    def delete(self, urns, options=None):
        ...
        if slivers:
            ...
            leases = self.shell.GetLeases(
                {'name': slice_name, 'node_id': node_ids})
    ...
    def shutdown(self, xrn, options=None):
        ...
        slices = self.shell.GetSlices(
            {'peer_id': None, 'name': slicename}, ['slice_id'])
        if not slices:
